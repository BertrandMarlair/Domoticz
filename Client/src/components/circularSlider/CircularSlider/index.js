...
import useIsServer from "../../../core/hooks/useIsServer";
...
const CircularSlider = ({
  ...
  const microSlide = async ({ degrees, pointsInCircle, offset }) => {
    ...
    const offsetRelativeToDocumentUp = (ref) => {
    ...
    const onMouseDown = (event) => {
        ...
        const docX = offsetRelativeToDocumentUp(circularSlider).left + state.radius;
        const docY = offsetRelativeToDocumentUp(circularSlider).top + state.radius;

        if (
            getDifference(docX, event.type === "touchstart" ? touch.pageX : event.pageX) < range &&
            getDifference(docY, event.type === "touchstart" ? touch.pageY : event.pageY) < range
        ) {
    ...
    const onClickMove = useCallback(
        (event, passive = false) => {
            ...
            const offsetRelativeToDocument = (ref) => {
                const rect = ref.current.getBoundingClientRect();
                const scrollLeft =
                    !isServer && ((window?.pageXOffset ?? 0) || (document?.documentElement?.scrollLeft ?? 0));
                const scrollTop =
                    !isServer && ((window?.pageYOffset ?? 0) || (document?.documentElement?.scrollTop ?? 0));
    ...
    useEffect(() => {
        let updatedIndex = activedItem === null ? value : value;
        ...
        if (dataArrayLength) {
            ...
            const degrees = getSliderRotation(direction) * knobPositionIndex * pointsInCircle;
            const radians = getRadians(degrees + 0.1) - getOffsetRideans(state.knobPosition, offsetAngle);
    ...
    useEventListener(SLIDER_EVENT.DOWN, onMouseDown);
